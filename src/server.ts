...
import { getTokenStoreKind } from "./token-store.js";
...
function tokenSavedMessage(): string {
  return getTokenStoreKind() === "redis"
    ? "Authentication successful. Tokens saved to cloud storage."
    : "Authentication successful. Tokens saved locally.";
}

export function createWhoopMcpServer(): McpServer {
  ...
  server.tool(
    "whoop_revoke_access",
    "Revoke WHOOP OAuth access and clear stored tokens",
    {},
    async () =>
      safeTool(async () => {
        await revokeAccess();
        return { success: true, message: "Access revoked and tokens cleared." };
      }),
  );
      ...
      safeTool(async () => {
        ...
        const [cycles, recoveries, sleeps, workouts] = await Promise.all([
          whoopClient.fetchAllPages(
            (p) =>
              whoopClient.listCycles(p) as Promise<{ records: unknown[]; next_token?: string }>,
            params,
            maxPages,
          ),
          whoopClient.fetchAllPages(
            (p) =>
              whoopClient.listRecoveries(p) as Promise<{ records: unknown[]; next_token?: string }>,
            params,
            maxPages,
          ),
          whoopClient.fetchAllPages(
            (p) =>
              whoopClient.listSleeps(p) as Promise<{ records: unknown[]; next_token?: string }>,
            params,
            maxPages,
          ),
          whoopClient.fetchAllPages(
            (p) =>
              whoopClient.listWorkouts(p) as Promise<{ records: unknown[]; next_token?: string }>,
            params,
            maxPages,
          ),
        ]);
